...
//        => optionsBuilder.UseSqlServer("Data Source=DESKTOP-R6LPKL1\\SQLEXPRESS;Initial Catalog=CViewer; Integrated Security=False;User ID=sa;Password=1234; TrustServerCertificate=True ");
...
//            entity.HasOne(d => d.Expert).WithMany(p => p.CvExperts)
...
//            entity.HasOne(d => d.People).WithMany(p => p.ProfileLogs)
...
//            entity.HasOne(d => d.People).WithMany(p => p.Reports)
//                .HasForeignKey(d => d.PeopleId)
//                .OnDelete(DeleteBehavior.ClientSetNull)
//                .HasConstraintName("FK_report_people");
